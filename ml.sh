...
cat << "EOF"
        ------------------------------------------------------
            ...
            Author : Aldy Nugraha (cccocot cccocot)
                            Recode : mtezy
        ------------------------------------------------------
...
function ngecek(){
  ...
  local stats="${PP}[$(date +"%T")]"
    ...
    if [[ $STATUS =~ "Error_Success" ]]; then
        printf "${CC} (${3}/${4}) ${empas} => [${OG}${B}LIVE${CC}] Checked Time: ${stats}\n"
        echo "${1}|${2}" >> mllive.txt
    elif [[ $STATUS =~ "Error_PasswdError" || $STATUS =~ "Error_NoAccount" ]]; then
        printf "${CC} (${3}/${4}) ${empas} => [${RD}${B}DIE${CC}] Checked Time: ${stats}\n"
        killall -HUP tor
    else
        printf "${CC} (${3}/${4}) ${empas} => [${CY}${B}UNK${CC}] Checked Time: ${stats}\n"
    fi
}
